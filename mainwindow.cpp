...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    table->setRows(QVector<QStringList>{
    ...
    connect(table, &TableWidget::tableSelectionChanged, this,
            [](int rows, int cols, QList<QString> rowData) {
                std::cout << "Selection changed" << std::endl;
            });

    table->setDoubleClickHandler([](int row, int col, const QList<QString>& rowData) {
        std::cout << "doubleclick handler" << std::endl;
    });

    connect(table, &TableWidget::rowUpdated, this,
            [](int row, int col, const QList<QString>& rowData) {
    ...
    writeQStringToFile(table->generateJsonData([](int col, QString value) -> QVariant {
        switch (col) {
            case 0:
                return value.toInt();
                break;
